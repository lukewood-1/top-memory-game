...
export default function CardContainer({difficulty, suit, onDifficultyChange}){
  ...
  useEffect(() => {
    ...
    const getCards = async () => {
      ...
      try {
        ...
        const capitalize = (target) => `${target.value.slice(0,1)}${target.value.slice(1).toLowerCase()} of ${target.suit.slice(0,1)}${target.suit.slice(1).toLowerCase()}`;
  ...
  return (<>
    ...
    <ScoreCounter score={score} deck={deck}/>
    <div className='card-container'>
      {showCards()}
    </div>
    <WinScreen onReset={resetGame} difficulty={difficulty} onDifficultyChange={onDifficultyChange} />
  </>)
}
